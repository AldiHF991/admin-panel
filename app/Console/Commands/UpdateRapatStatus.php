...
class UpdateRapatStatus extends Command
{
    ...
    public function handle()
    {
        ...
        // PERBAIKAN: Status "Diterima/Disetujui" seharusnya memiliki id_status = 2
        // - Waktu sekarang >= waktu mulai DAN Waktu sekarang < waktu selesai
        $rapatBerlangsung = Rapat::where('id_status', 1) // Disetujui
        ...
        // 2. Ubah status menjadi "Selesai" (id_status = 3) dari rapat yang "Berlangsung" atau "Disetujui"
        // PERBAIKAN: id_status untuk "Selesai" diubah menjadi 3 agar tidak bentrok dengan "Ditolak" (ID 5)
        // Kondisi:
        // - Status saat ini adalah "Berlangsung" (4) ATAU "Disetujui" (2)
        ...
        $rapatSelesai = Rapat::whereIn('id_status', [1, 4]) // Disetujui atau Berlangsung
            ->whereRaw('? >= CONCAT(tanggal, " ", waktu_end)', [$now])
